<?php
 ...
 <html>
   <head>
     ...
     <title>Bank of Wakanda CPMS  </title>
 ...
 <?php
   ...
   while( $arry=mysql_fetch_array($query1) ) {
     ...
     $ref = $arry['ref_no'];
      ...
      echo "<tr> <td> <b> Complaint No. </b> </td>";
      echo "     <td> ".$ref."</td></tr>";
   ...
   if(($status)==46){
     $query = mysql_query("SELECT ref_no FROM `view_cmp` WHERE ref_no=$ref");
       if (mysql_num_rows($query) != 0){
           #$message =  "This message is already send to the selected Bank Officer";
         ...
         <div class ='text-right' id='close'>
          ...
          <p>This message is already send to the selected Bank Officer</p>
         </div>";
         ...
         <svg class='pointer' fill='#FFF' height='24' viewBox='0 0 24 24' width='24' xmlns='http://www.w3.org/2000/svg'>
           ...
           <p class='text-center'>Message successfully sent to the selected Bank Officer</p>
         </div> </div>";
       }
   }else{
          $message = "<div class='alert errr' id='msg'>
